...
def solve(grid: List[List[str]]) -> Optional[List[List[str]]]:
    ...
    if not pos:
        return grid
    row, col = pos
    for s in find_possible_values(grid, pos):
        grid[row][col] = s
        solute = solve(grid)
        if solute:
            return solute
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    for s in range(len(solution)):
        solved = set(get_row(solution, (s, 0)))
        if solved != set("123456789"):
            return False
    for j in range(len(solution)):
        solved = set(get_col(solution, (0, j)))
        if solved != set("123456789"):
            return False
    for s in range(0, len(solution), 3):
        for j in range(0, len(solution), 3):
            solved = set(get_block(solution, (s, j)))
            if solved != set("123456789"):
                return False
    return True
